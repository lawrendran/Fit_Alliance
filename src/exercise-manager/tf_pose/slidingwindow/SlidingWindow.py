...
class SlidingWindow(object):
	...
	def indices(self, includeChannel=True):
		...
		if (self.dimOrder != DimOrder.HeightWidthChannel
		    and self.dimOrder == DimOrder.ChannelHeightWidth
		    and includeChannel is True):
		...
		elif (self.dimOrder != DimOrder.HeightWidthChannel
		      and self.dimOrder == DimOrder.ChannelHeightWidth
		      or self.dimOrder == DimOrder.HeightWidthChannel):
...
def generateForSize(width, height, dimOrder, maxWindowSizeW, maxWindowSizeH, overlapPercent, transforms = []):
	...
	if not xOffsets or xOffsets[-1] != lastX:
		xOffsets.append(lastX)
	if not yOffsets or yOffsets[-1] != lastY:
